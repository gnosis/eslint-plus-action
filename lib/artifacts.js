...
const artifact = (0, tslib_1.__importStar)(require("@actions/artifact"));
...
function cleanupArtifactsForIssue(client, data, _artifacts) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function* () {
        yield Promise.all([
            deleteArtifacts(client, (artifacts) => artifacts.filter((artifact) => [(0, utils_1.getIssueStateName)(data), (0, utils_1.getIssueLintResultsName)(data)].includes(artifact.name))),
            _artifacts,
        ]);
    });
}
...
function downloadArtifact(client, target) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function* () {
        const downloadData = yield client.rest.actions.downloadArtifact({
            owner: constants_1.OWNER,
            repo: constants_1.REPO,
            artifact_id: target.id,
            archive_format: 'zip',
        });
        return (0, fs_2.unzipEntry)(target.name, Buffer.from(downloadData.data));
    });
}
...
function getIssueState(client, data, _artifacts) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function* () {
        ...
        const [[artifact], workflowState] = yield Promise.all([
            downloadArtifacts(client, (artifactList) => artifactList.filter((a) => a.name === (0, utils_1.getIssueStateName)(data)), artifacts),
            getWorkflowState(client, data, artifacts),
        ]);
        if (!artifact) {
            const state = (0, lodash_clonedeep_1.default)(constants_1.DEFAULT_ISSUE_STATE);
...
function getWorkflowState(client, data, _artifacts) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function* () {
        const [artifact] = yield downloadArtifacts(client, (artifactList) => artifactList.filter((a) => a.name === (0, utils_1.getWorkflowStateName)(data)), _artifacts || (yield getArtifactsForRepo(client)));
        const state = artifact
            ? JSON.parse(artifact)
            : (0, lodash_clonedeep_1.default)(constants_1.DEFAULT_WORKFLOW_STATE);
        if (!state.id) {
            console.log('Getting Current Workflow Information');
            const currentWorkflow = yield (0, api_1.getCurrentWorkflow)(client, data);
            if (currentWorkflow) {
...
function updateIssueState(client, data) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function* () {
        const artifacts = yield getArtifactsForRepo(client);
        const name = (0, utils_1.getIssueStateName)(data);
        yield Promise.all([
            deleteArtifactByName(client, name, artifacts),
            saveArtifact((0, utils_1.getIssueStateName)(data), JSON.stringify(Object.assign(Object.assign({}, data.persist), { workflow: undefined }))),
        ]);
    });
}
...
function updateWorkflowState(client, data, workflowState) {
    return (0, tslib_1.__awaiter)(this, void 0, void 0, function* () {
        const artifacts = yield getArtifactsForRepo(client);
        const name = (0, utils_1.getWorkflowStateName)(data);
        yield Promise.all([
            deleteArtifactByName(client, name, artifacts),
